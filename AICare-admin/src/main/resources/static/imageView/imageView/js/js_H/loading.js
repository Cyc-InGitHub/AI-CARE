  ...
  if (views.length ===1){
    views.forEach(function(axis_name) {
      ...
      if(axis_name==="xspace"){
        ...
        document.querySelector("div.viewport.ui-droppable canvas[class='cornerstone-canvas']").remove()
        var element= document.querySelector("div.viewport.ui-droppable");
        //cornerstone.disable(element);
        cornerstone.enable(element);
        // var canvas1 = document.createElement('canvas');
        // canvas1.style.display = 'block';
        // canvas1.classList.add('cornerstone-canvas');
        // element.appendChild(canvas1);
        canvas= document.querySelector("div.viewport.ui-droppable canvas[class='cornerstone-canvas']")
        volume.width = default_panel_width;
        volume.height = default_panel_height;

        var image={

          color:false,
          columnPixelSpacing: 0.859375,
          columns: 256,
          data:volume.data,
          decodeTimeInMS: 0,
          floatPixelData: undefined,
          getPixelData: volume.getPixelData,
          height: 20,
          intercept: 0,
          invert: false,
          loadTimeInMS: 4314,
          maxPixelValue: 16157,
          minPixelValue: 0,
          rowPixelSpacing: 7.071067810058594,
          rows: 20,
          sizeInBytes: 10240,
          slope: 1,
          //totalTimeInMS: 7488,
        ...
        cornerstone.displayImage(element, volume);
...
if(axis_name==="yspace")
    ...
    var div_brain =  document.getElementById("control");
    div_brain.appendChild(container);
