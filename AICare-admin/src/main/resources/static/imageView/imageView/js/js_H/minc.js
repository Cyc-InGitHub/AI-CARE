...
(function() {
  ...
  VolumeViewer.createVolume = function(header,native_data) {
    ...
    var volume = {
      ...
      maxPixelValue: header.voxel_max,
      minPixelValue: header.voxel_min,
      windowCenter: header.voxel_max/2-header.voxel_max/4,
      windowWidth:  header.voxel_max/2+header.voxel_max/4,
      slope: header.scl_slope,
      columnPixelSpacing: Math.abs(header.zspace.step),
      columns: header[header.order[0]].space_length,
      rowPixelSpacing: Math.abs(header.yspace.step),
      rows: header[header.order[2]].space_length,
      height: 20,
      width: 512,
      //sizeInBytes: header.bytes_per_voxel,
      decodeTimeInMS: 0,
      //render: renderGrayscaleImage(enabledElement, invalidated),
      ...
      slice: function(axis, slice_num, time) {
        ...
        //调用卷数据的构造函数可以保证切片数据缓冲区与卷具有相同的类型。
        ...
        // Whether the dimension steps positively or negatively.维度是正向的还是负向的
        var x_positive = width_space.step  > 0;
        var y_positive = height_space.step > 0;
        var z_positive = axis_space.step   > 0;
        // iterator for the result slice.结果切片的迭代器
        var i = 0;
console.log(axis_space_offset,height_space_offset,width_space_offset);
        z = z_positive ? slice_num : axis_space.space_length - slice_num;
      ...
      // Calculate the world to voxel transform and save it, so we can access it efficiently
      // 计算世界到体素的变换并保存它，所以我们可以有效地访问它. The transform is:
    ...
    // for(var hm=3550000; hm<3565000; ++hm){
    //   volume.data[hm] = volume.data[hm] >=0 ? 255: 0;
    // }
    // for(var hm=4555000; hm< native_data.length; ++hm){
    //   volume.data[hm] = volume.data[hm] >=0 ? 0: 0;
    // }
    // for(var hm=4530399; hm<4587296; ++hm){
    //   volume.data[hm] = 255;
    // }
    // volume.data[5789784]=1101;

    // saveShareContent(volume.data,"hmz.mnc.raw");
