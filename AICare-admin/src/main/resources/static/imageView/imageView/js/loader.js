  ...
  var loader = BrainBrowser.loader = {
    ...
    */
    loadFromFile: function(file_input, callback, options) {
      ...
      //var filename = parts[parts.length-1];
      var filename =file_input[0].name;
    ...
    */
    ...
    /**
     * @doc function
     * @name BrainBrowser.loader:loadFromFiles
     * @param {DOMElement}  files File input DOM object.
     * @param {function} callback
     * @param {object} options Any options are passed on to the **callback** function.
     * @description
     */
    loadFromFiles: function(file_input,callback,options){
      //var files = file_input.files;
      var files = file_input;
      var filename=[];
      var allFiles=[];
      ...
      if(files.length<=1){
        ...
        for(var file of files){
          ...
          reader.onloadend = function(event) {
            var result = event.target.result;//文件内容
            try {
              /* See if the data can be inflated.
               */
              var unzipped = pako.inflate(result);
              ...
              i++;
              if(i==files.length){
                callback(allFiles,filename, "dicom_files", options);
              }
            }
          };
          //该事件在读取操作发生错误时触发。
          reader.onerror = function() {
            var error_message = "error reading file: " + filename[i];
    ...
    /**
     * @doc function
     * @name BrainBrowser.loader:loadFromUrls
     * @param {DOMElement}  files urls input DOM object.
     * @param {function} callback
     * @param {object} options Any options are passed on to the **callback** function.
     * @description
     */   //  -------------------------------------------- 暂时有未解决的问题（向服务器请求多个文件），-------------------------------
    loadFromUrls: function(file_urls,callback,options){
      var filename=[];
      var allFiles=[];
      var i=0;//计数
      ...
      if(file_urls.length<=1){
        ...
        for(var file of file_urls){
          var parts = file.imageId.split("/");
          filename[i] = parts[parts.length-1];
          ...
          request.open("GET", file.imageId);

          request.onreadystatechange = function() {
            if (request.readyState === 4){
              status = request.status;

              allFiles[i]=request.response;
              i++;
              ...
              if(status >= 200 && status < 300 || status === 304) {
                ...
                var error_message = "error loading URL: " + file.imageId + "\n" +
                    "HTTP Response: " + request.status + "\n" +
                    "HTTP Status: " + request.statusText + "\n" +
                    "Response was: \n" + request.response;
