...
function setupUi() {
  ...
  if (_data.volume.file.length > 0) {
    ...
    //添加的旋转图像推动条
    jQuery('#rotationX0_0Slider').slider("option", "value", 50);
    jQuery('#rotationY0_0Slider').slider("option", "value", 50);
    jQuery('#rotationZ0_0Slider').slider("option", "value", 50);
...
// //添加的旋转图像拖动条触发事件函数
function rotate3dVolume(event, ui) {
  var rotateView = ui.handle.offsetParent.id;
  MenuItemSlider.action =rotateView;
  var rotate= 1.0 - ui.value / 100;
  
  if (rotateView == "rotationX0_0Slider"){
    //MenuItemSlider.rotate[0] = rotate;
    MenuItemSlider.rotationX = rotate;
  }else if (rotateView == "rotationY0_0Slider"){
    //MenuItemSlider.rotate[1] = rotate;
    MenuItemSlider.rotationY = rotate;
  }else {
    //MenuItemSlider.rotate[2] = rotate;
    MenuItemSlider.rotationZ = rotate;
  }
...
function updateTransform() {
  var rotX = (MenuItemSlider.rotationX - 0.5) * 180,
      rotY = (MenuItemSlider.rotationY - 0.5) * 180,
      rotZ = (MenuItemSlider.rotationZ - 0.5) * 180,
      centerX, centerY, centerZ;

  centerX = (volume.ca[0] / 2) * Math.abs(viewer.volumes[0].header.xspace.step);
  centerY = (volume.ca[1] / 2) * Math.abs(viewer.volumes[0].header.yspace.step);
  centerZ = (volume.ca[2] / 2) * Math.abs(viewer.volumes[0].header.zspace.step);
