...
$(function () {
    ...
    // .button() 这个方法是Jquery-UI.js带的 会让<button>加上jquery-UI的按钮样式
    ...
    window.viewer = BrainBrowser.VolumeViewer.start("brainbrowser", function (viewer) {
        ...
        // Should cursors in all panels be synchronized?  synchronized 功能
        ...
        $("#volume-file-nifti1-submit1").click(function () {
            this.div = document.getElementById('container');
            $(this.div).remove();
            viewer.volumes.forEach(function (volume) {
                volume.triggerEvent("eventmodelcleanup");
            ...
            viewer.mask = 0;//全局变量，在调制↑，↓键监听事件时使用，用来判断得到是哪个canvas的监听事件
            viewer.select = 0;
     ...
     */
       ...
       */
        ...
        ////////////////////////////////////////
        viewer.setDefaultPanelSize(350, 350);
        ///////////////////
        // Start rendering.
        ///////////////////
        viewer.render();
    ...
    scr.onmousewheel = function (e) {
        ...
        if (ev.wheelDelta > 0) {
            var length = viewer.volumes.length - 1;
            var volume = viewer.volumes[length];
            if (volume.position["xspace"] + 1 <= volume.header["xspace"].space_length) {
                volume.position["xspace"] = volume.position["xspace"] + 1;
                volume.display.forEach(function (other_panel) {
                    if (other_panel.axis === "xspace") {
                        other_panel.updateSlice();
    ...
    function roll(y) {
        ...
        var length = viewer.volumes.length - 1;
        var volume = viewer.volumes[length];
        volume.position["xspace"] = Math.round(y / viewer.spacing[0]);
        volume.display.forEach(function (other_panel) {
            if (other_panel.axis === "xspace") {
                other_panel.updateSlice();
            }
    ...
    function roll1(y) {
        ...
        var length = viewer.volumes.length - 1;
        var volume = viewer.volumes[length];
        volume.position["yspace"] = Math.round(y / viewer.spacing[1]);
    ...
    function roll2(y) {
        ...
        var length = viewer.volumes.length - 1;
        var volume = viewer.volumes[length];
        volume.position["zspace"] = Math.round(y / viewer.spacing[2]);
    ...
    scr2.onmousewheel = function (e) {
        ...
        if (ev.wheelDelta > 0) {
            var length = viewer.volumes.length - 1;
            var volume = viewer.volumes[length];
            if (volume.position["yspace"] + 1 <= volume.header["yspace"].space_length) {
                volume.position["yspace"] = volume.position["yspace"] + 1;
    ...
    scr3.onmousewheel = function (e) {
        ...
        if (ev.wheelDelta > 0) {
            var length = viewer.volumes.length - 1;
            var volume = viewer.volumes[length];
            if (volume.position["zspace"] + 1 <= volume.header["zspace"].space_length) {
                volume.position["zspace"] = volume.position["zspace"] + 1;
