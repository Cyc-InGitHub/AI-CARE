...
(function() {
  ...
  VolumeViewer.volume_loaders.overlay = function(options, callback) {
    ...
    for (var i = 0; i < order.length; i++) {
      ...
      header[order[i]].start = - header[order[i]].space_length/ 2;
    ...
    overlay_volume.position=volumes[0].position;
    ...
    overlay_volume.getSliceImage = function(slice, zoom, contrast, brightness) {
      ...
      var volume = overlay_volume.volumes[0];
      ...
      var yh = 350/ y_fov;
      var zh = 350 / z_fov;
     var zoom1=Math.min(yw, xw, zh, yh);
      zoom = zoom || zoom1;
     ...
     slices.forEach(function(slice, i) {
        ...
        var yh = 350/ y_fov;
        var zh = 350 / z_fov;
        // zoom=Math.min(yw, xw, zh, yh);
        var image1 = volume.getSliceImage(slice, zoom, contrast, brightness);
        var xstep = slice.width_space.step;
        var ystep = slice.height_space.step;
        max_width = Math.abs(Math.floor(slice.width * xstep * zoom));
        max_height = Math.abs(Math.floor(slice.height * ystep * zoom));
  ...
  function blendImages(images, blend_ratios, target) {
    var num_images = images.length;//2
