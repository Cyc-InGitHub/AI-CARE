  ...
  */
  BrainBrowser.createTreeStore = function() {
    ...
    return {
      ...
      */
      set: function() {
        var value = arguments[arguments.length - 1];   //arguments[]为当前函数的参数列表   //取最后一个元素，为结点的值
        var keys = Array.prototype.slice.call(arguments, 0, arguments.length - 1); //除最后一个外的全部结点的关键字
        var subtree = tree;
        var current_key;
        var i, count;
        var error_message;
        ...
        for (i = 0, count = keys.length - 1; i < count; i++) {
          current_key = keys[i];
          if (subtree[current_key] && typeof subtree[current_key] !== "object") { //存在该结点，且不为一个对象
