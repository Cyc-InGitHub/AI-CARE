  ...
  */
  VolumeViewer.start = function(element, callback) {
    ...
    */
    ...
    Object.keys(VolumeViewer.modules).forEach(function(m) {  //为viewer注册加载函数和渲染函数
      VolumeViewer.modules[m](viewer);
    });

    BrainBrowser.events.addEventModel(viewer);  //为viewer注册活动处理程序
    ...
    function keyboardControls() {
      document.addEventListener("keydown", function(event) {
        ...
        var keys = {
          // CTRL
          17: function() {
            if (panel.anchor) {  //panel.anchor 当测量距离时，记录起始位置
              return;
            }
